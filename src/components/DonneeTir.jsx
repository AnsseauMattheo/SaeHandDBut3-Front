...
const DonneTir = ({ tirs, totalTirs = 0, tirsReussi = 0, secteur, reset, updateReset, data = true }) => {
    ...
    if (data) {
        return (
            <div
                className={`inline-flex items-center bg-slate-900 cursor-pointer rounded-lg px-1.5 sm:px-2 py-1.5 sm:py-2 text-white text-[10px] sm:text-xs lg:text-sm font-medium ${showInfo ? "z-10" : ""}`}
                onClick={handleClick}
            >
                {!showInfo && (
                    <>
                        <div className="relative flex items-center mr-0.5 sm:mr-1">
                            <div className="w-7 h-7 sm:w-8 sm:h-8 lg:w-9 lg:h-9 bg-gradient-to-r from-red-500 to-pink-500 rounded-full flex items-center justify-center">
                <span className="text-white text-[9px] sm:text-[10px] lg:text-xs font-bold">
                  {totalTirs > 0 ? Math.round((tirs / totalTirs) * 100) + "%" : "0%"}
                        ...
                        <div className="flex items-center bg-blue-600 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-[9px] sm:text-xs">
    ...
    return (
      ...
      `}
        ...
        >
            {showInfo && (
                <div className="-top-12 sm:-top-16 left-1/2 -translate-x-1/2 z-10 bg-slate-900/95 text-white text-[9px] sm:text-xs rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 shadow-lg whitespace-nowrap border border-slate-700">
