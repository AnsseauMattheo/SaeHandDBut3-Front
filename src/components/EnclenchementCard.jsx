...
const EnclenchementCard = ({ stats, rank, onClick }) => {
    ...
    return (
        ...
        >
            {rank && (
                <div className={`absolute -top-3 -left-3 w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-lg z-2 bg-blue-300 }`}>
                    #{rank}
                </div>
            )}
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                        <div className="text-xs text-gray-400">Effectués</div>
                        <div className="text-lg font-bold">{stats.effectues}</div>
                    </div>
                    <div>
                        <div className="text-xs text-gray-500">Réussis</div>
                        <div className="text-lg font-bold text-green-500">{stats.reussis}</div>
                    </div>
                    <div>
                        <div className="text-xs text-gray-500">Échecs</div>
                        <div className="text-lg font-bold text-red-400">{stats.echecs}</div>
                    </div>
                </div>
